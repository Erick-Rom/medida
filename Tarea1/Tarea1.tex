...
Renata Capistrán Fuentes- 2079992\\
Diego Sanchez- \\
...
\begin{itemize}
    \item[$\star$] \textbf{El vacío está en $\mathcal{A}$:}
    ...
    \item[$\star$] \textbf{Cerradura bajo complementos:}
    ...
    \item[$\star$] \textbf{Cerradura de uniones numerables:}
...
\begin{itemize}
    ...
    El resultado se sigue directo de las leyes de Morgan.

    \item Si A es $G_\delta$, entonces $A^c=F_\sigma$
    
    De igual forma se sigue el resultado por las leyes de Morgan.

    \item La unión numerable de conjuntos $F_\sigma$ es $F_\sigma$
    
    La union de los elementos de una coleccion, cuyos elementos son uniones de cerrados, a su vez la union de uniones de cerrados, puede ser vista como una propia union de cerrados.

    \item La intersección numerable de conjuntos $G_\delta$ es $G_\delta$
    
    La intersección de los elementos de una colección, cuyos elementos son intersecciones de cerrados, a su vez la intersección de intersecciones de cerrados, puede ser vista como una propia intersección de cerrados.
\end{itemize}
...
\begin{itemize}
    \item[$\star$] Medida del vacío:
    Por vacuidad, sabemos que el conjunto vacío no contiene elementos, luego $x_0 \notin \emptyset$. Por la definición de nuestra función, esto implica directamente que:
    $$\mu_{x_0}(\emptyset) = 0$$

    \item[$\star$] $\sigma$-aditividad:
    Sea $\{E_n\}_{n \in \mathbb{N}}$ una sucesión de conjuntos en $S$ ajenos por pares (es decir, $E_i \cap E_j = \emptyset$ para $i \neq j$). Queremos demostrar que $\mu_{x_0}\left(\bigcup_{n \in \mathbb{N}} E_n\right) = \sum_{n \in \mathbb{N}} \mu_{x_0}(E_n)$.
    
    Analizamos por casos:
    
    \textbf{Caso 1: Suponga que $x_0 \notin E_n \forall n \in \mathbb{N}$.} \\
    Si $x_0$ no está en ningún conjunto de la sucesión, entonces tampoco está en su unión, por lo que $x_0 \notin \bigcup_{n \in \mathbb{N}} E_n$. 
    Evaluando la función, tenemos que $\mu_{x_0}\left(\bigcup_{n \in \mathbb{N}} E_n\right) = 0$. 
    Por otro lado, la suma de las medidas es $\sum_{n \in \mathbb{N}} \mu_{x_0}(E_n) = \sum_{n \in \mathbb{N}} 0 = 0$. 
    Ambos lados son iguales a cero, por lo que la igualdad se cumple.
    
    \textbf{Caso 2: Suponga que existe un índice $k \in \mathbb{N}$ tal que $x_0 \in E_k$.} \\
    Al pertenecer a $E_k$, evidentemente $x_0 \in \bigcup_{n \in \mathbb{N}} E_n$, lo que implica que la medida de la unión es $\mu_{x_0}\left(\bigcup_{n \in \mathbb{N}} E_n\right) = 1$.
    Ahora, como los conjuntos son \textbf{ajenos por pares}, $x_0$ no puede pertenecer a ningún otro conjunto $E_j$ con $j \neq k$. Por lo tanto, $\mu_{x_0}(E_k) = 1$ y $\mu_{x_0}(E_j) = 0$ para todo $j \neq k$. 
    Al hacer la suma infinita obtenemos:
    $$\sum_{n \in \mathbb{N}} \mu_{x_0}(E_n) = \mu_{x_0}(E_k) + \sum_{j \neq k} \mu_{x_0}(E_j) = 1 + 0 = 1$$
    Ambos lados de la ecuación son iguales a 1, demostrando la igualdad.
\end{itemize}
\paragraph{7}
...
Entonces $t+E=\emptyset\in\mu$

$\therefore\emptyset\in A$

Si $t+E\in A$

$(t+E)^c=t+E^c\in A$ Ya que $E^c\in\mu$

Sea $\{t+E_n\}_{n\in\mathbb{N}}\subset A$

Tenemos que $\bigcup\limits_{n\in\mathbb{N}}(t+E_n)=t+\bigcup\limits_{n\in\mathbb{N}}E_n\in A$
...
Supongamos que $\mathbb{R}-\bigcup\limits_{n\in\mathbb{N}}(r_n-\frac{1}{n^2},r_n+\frac{1}{n^2})=\emptyset$

Entonces se sigue que $\mathbb{R}=\bigcup\limits_{n\in\mathbb{N}}(r_n-\frac{1}{n^2},r_n+\frac{1}{n^2})$

Entonces $\infty=m(\mathbb{R})=m(\bigcup\limits_{n\in\mathbb{N}}(r_n-\frac{1}{n^2},r_n+\frac{1}{n^2}))=\sum\limits_{n\in\mathbb{N}}\frac{2}{n^2}= L<\infty$ lo cual es una contradicción.
\end{document}
